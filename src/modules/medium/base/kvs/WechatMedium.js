...
Kv.prototype.loadById = function(id, callback){
    ...
    redis.hgetall(key, function(err, result){
        cbUtil.logCallback(
            err,
            'Fail to get wechat medium by id ' + id + ': ' + err,
            'Succeed to get wechat medium by id ' + id);
...
Kv.prototype.saveById = function(json, callback){
    ...
    redis.hmset(key, json, function(err, result){
        cbUtil.logCallback(
            err,
            'Fail to save wechat medium by id ' + id + ': ' + err,
            'Succeed to save wechat medium by id ' + id);
